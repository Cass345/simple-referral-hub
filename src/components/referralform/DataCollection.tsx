...
import type { Behavior, DataCollectionMethod } from '@/types/referral';
...
export function DataCollection({ behaviors, onSubmit, onBack, initialData = {} }: DataCollectionProps) {
  const [dataCollection, setDataCollection] = useState<Record<number, DataCollectionMethod>>(initialData);
  ...
  const updateDataCollection = (behaviorId: number, field: keyof DataCollectionMethod, value: string | string[]) => {
    ...
    <div className="bg-white rounded-lg shadow-lg p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {behaviors.map((behavior, index) => {
          const method = dataCollection[index] || { type: '', frequency: '', tools: [], notes: '' };
          ...
          return (
            <div key={index} className="p-4 border border-gray-200 rounded-lg">
              ...
              <div>
                ...
                <select
                  value={method.type}
                  onChange={(e) => updateDataCollection(index, 'type', e.target.value)}
                ...
                {method.type && (
                  <div className="mt-1 text-sm text-gray-500">
                    <p>{dataCollectionTypes.find((t) => t.value === method.type)?.description}</p>
                    <p className="italic">{dataCollectionTypes.find((t) => t.value === method.type)?.example}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateDataCollection(index, 'frequency', e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
              ...
              {method.type && (
                <div>
                  ...
                  <div className="space-y-2">
                    {dataCollectionTypes
                      .find((t) => t.value === method.type)
                      ?.tools.map((tool) => (
                        <label key={tool} className="flex items-center">
                          <input
                            type="checkbox"
                            checked={method.tools?.includes(tool)}
                            onChange={(e) => {
                              const tools = method.tools || [];
                              const newTools = e.target.checked
                                ? [...tools, tool]
                                : tools.filter((t) => t !== tool);
                              updateDataCollection(index, 'tools', newTools);
                            }}
              ...
              <div>
                ...
                <textarea
                  value={method.notes}
                  onChange={(e) => updateDataCollection(index, 'notes', e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  rows={3}
        ...
        <div className="flex justify-between mt-6">
          ...
          <button
            type="submit"
            disabled={!Object.values(dataCollection).every(isMethodComplete)}
